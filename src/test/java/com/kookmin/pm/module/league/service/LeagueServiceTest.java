...
class LeagueServiceTest {
    ...
    @Test
    @DisplayName("startLeague 메소드 성공 테스트")
    public void startLeague_success_test() {
        Member host = memberRepository.findByUid("dlwlsrn9412@kookmin.ac.kr").get();
        Member participant = memberRepository.findByUid("dlwlsrn10@kookmin.ac.kr").get();
        ...
        leagueService.startLeague(host.getId(), id);

        League league = leagueRepository.findById(id).get();

        assertThat(league)
                .hasFieldOrPropertyWithValue("status", LeagueStatus.PROCEEDING);
    }
}